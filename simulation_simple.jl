...
struct Bridge
    L::Int
    tempered_glass_panel::Array{Bool, 2} # true. 2 × L
...
function Bridge(L::Int)
    # build the path
    glass_panels = falses(2, L)
    for step in 1:L # loop over steps along path
        # label the safe spots
        safe_panel = sample(1:2)
        glass_panels[safe_panel, step] = true
    end

    # initialize the players' view of the game's state
    observed = [missing for _ in 1:L]
    
    return Bridge(L, glass_panels, observed)
end

solved(bridge::Bridge) = ! ismissing(bridge.observed[bridge.L]) # adrian likes this one
...
function attempt_crossing!(bridge::Bridge)
    ...
    # store history; this column is observed
    bridge.observed[next_column_to_hop_to] = panel_to_hop_on
    
    if bridge.tempered_glass_panel[panel_to_hop_on, next_column_to_hop_to]
...
Returns the number of players that survived.
"""
function simulate_hopscotch(L::Int, nb_players::Int)
	# initialize the playing field
    bridge = Bridge(L)
	
	# loop over players and have each one make guesses
	for player in 1:nb_players
		survived = attempt_crossing!(bridge) # make guesses until a stopping condition is met
        if solved(bridge)
            return survived ? nb_players - player + 1 : nb_players - player
        end
	end
