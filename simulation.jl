...
A special game of hopscotch is devised wherein $n$ players sequentially attempt to traverse a path of length $n$.
At each step of the path, the player must choose how to proceed: on the left, or on the right.
Each choice is a 50/50 gamble on whether the player hits a dead end and is eliminated from the game.

!!! note
	this would be a good place for a figure

What is the probability of a given player traversing the path?

Assume that each player will make guesses until either reaching the end of the playing field or making an incorrect guess and players will proceed in their assigned order.
Also assume no mistakes are made (each player follows the solved path as far as it is known).

We can solve this problem via Monte-Carlo simulation.

## The Playing Field

We define a `struct Field` to track the state of the game.
Its `path` attribute encodes the safe/unsafe spaces as true/false.
The path is as long as `length` (number of steps to cross) and as wide as `width` (number of options at each step).
The `state` attribute encodes the players' knowledge of the game's state; `missing` values show which guesses haven't been made yet.
The `solved` attribute tracks how many steps along the path have been solved (meaning there is a safe choice known for that step).
We initialize a `Field` by passing it a length, width, and number of safe choices per step.
"""

# ╔═╡ c826a0a0-c034-4b33-aaee-2fc26a361e67
begin
	# data type for tracking the state of the playing field
	mutable struct Field
		path::Array{Bool} # True = part of correct path
		length::Int # how long the path is
		width::Int # how many "lanes" wide the field is
		solved::Int # how many steps of the correct path are known
		state::Array{Union{Bool, Missing}} # state of the game as known to players
	end
	...
	function Field(steps::Int, lanes::Int, safe::Int)
		...
		for step in 1:steps # loop over steps along path
			# label the safe spots
			safe_lanes = Int.(sample(1:lanes, safe, replace=false))
			path[step, safe_lanes] .= true
		end
		
		# initialize the players' view of the game's state
		state = reshape([missing for _ in 1:length(path)], (steps, lanes))
		
		return Field(path, steps, lanes, 0, state)
	end
end;
...
function attempt_crossing!(field::Field)
	...
	while !solved(field)
		# start from the first step w/o a known safe spot
		position = field.solved + 1
		# get the possible choices at this step
		options = [i for i in 1:field.width if ismissing(field.state[position, i])]
		# make a selection
		guess = sample(options)
		
		if field.path[position, guess]
			# good guess -- mark the correct step and move on
			field.state[position, guess] = true
			field.solved += 1
		else
			# bad guess -- mark the incorrect step and end this player's turn
			field.state[position, guess] = false
			break
		end
	end
end;
...
function simulate_hopscotch(players::Int, steps::Int, lanes::Int, safe::Int)::Int
	...
	# loop over players and have each one make guesses
	for player in 1:players
		attempt_crossing!(field) # make guesses until a stopping condition is met
		if solved(field)
			# if player m made it, then n - m + 1 players made it
			return players - player + 1
		end
	end
	
	# if final player didn't make it, no one did
	return 0
end;
